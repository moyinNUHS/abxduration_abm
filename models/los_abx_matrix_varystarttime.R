...
# 1. define abx.type (instead of using p.r.after) - such that courses are for 1 type of antibiotics only 
# 2. generates 2 abx matrixes - one with antibiotics starting on admission and one with antibiotics starting during admission - since los is the same
#    in both arms
...
los_abx_table_starttime <- function(n.bed, n.day, max.los, abx.type, 
                                    p.infect, cum.r.1,
                                    meanDur, timestep){
  ...
  all_abx = lapply(patient.id, function(id){ # for each patient 
    ...
    # get start time day 1 of admission vs later
    if (sum(fill) > 0) {
      
      total.dur = sum(fill == abx.type)
      fill.startonadmission = fill.startlater = rep(0, length(fill))
      fill.startonadmission[1 : (total.dur - 1)] = abx.type
      fill.startlater[(length(fill) - total.dur + 1) : length(fill)] = abx.type
      
    } else {
      fill.startonadmission = fill.startlater = fill
    }
    
    return(list(fill.startonadmission = fill.startonadmission, fill.startlater = fill.startlater))
  })
  
  all_abx_startonadmission = lapply(all_abx, `[[`, 1)
  all_abx_startlater = lapply(all_abx, `[[`, 2)
  ...
  all_los = lengths(all_abx_startonadmission) #update length of stay incorporating abx given after admission
  patient.matrix = abx.matrix.startonadmission = abx.matrix.startlater = matrix(NA, nrow = n.day, ncol = n.bed)
  idx = 1
  for(j in 1:n.bed){
    ...
    abx.matrix.startonadmission[,j] = unlist(all_abx_startonadmission[idx:end.pt.id])[1:n.day]
    abx.matrix.startlater[,j] = unlist(all_abx_startlater[idx:end.pt.id])[1:n.day]
    
    idx = end.pt.id + 1
  }
  
  patient.matrix = patient.matrix[rep(1:n.day, each = timestep), ]
  abx.matrix.startonadmission = abx.matrix.startonadmission[rep(1:n.day, each = timestep), ]
  abx.matrix.startlater = abx.matrix.startlater[rep(1:n.day, each = timestep), ]
  
  return(list(patient.matrix, abx.matrix.startonadmission, abx.matrix.startlater))
}
