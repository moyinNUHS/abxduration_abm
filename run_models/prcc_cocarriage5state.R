...
run_prcc_cocarriage <- function(abx.r) {
    ...
    modelRun.cocarriage <- function (data.df) { 
        return(mapply(run_absdiff_cocarriage5state, 
    ...
    parameters <- list(
        ...
        p.infect.after = c("qunif", list(min=0.01, max=0.13)),        #admission day when cumulative prabability of HAI requiring abx.r is 1
    ...
    if(all(names(parameters) == formalArgs(run_absdiff_cocarriage5state))){
        ...
        stop("Error: Listing of parameters in `parameters` does not match that in run_absdiff_cocarriage5state function.")
    }
...
run_prcc_cocarriage(0.5) 
run_prcc_cocarriage(0.00001) 
