...
cl <- makeCluster(detectCores(), outfile=paste0('error_files/parallel_error_populationgrowth_treated_', Sys.Date(), '.txt'))

# source functions on all cores
clusterCall(cl, function() {source('models/get_output_treated_populationgrowth.R')})
...
modelRun.populationgrowth <- function (data.df) { #data.df is a dataframe of the parameter values in columns 
  return(mapply(run_treated_populationgrowth, 
...
source('models/get_output_treated_populationgrowth.R')
if(all(names(parameters) == formalArgs(run_treated_populationgrowth))){
 ...
 # print('calculating 2nd run')
  ...
  stop("Error: Listing of parameters in `parameters` does not match that in run_treated_populationgrowth function.")
}
