...
library(parallel)
cl <- makeCluster(detectCores(), outfile = paste0('error_files/parallel_error_cocarriage5state_varycourse_', Sys.time(), '.txt'))

################################### Dependencies and functions ################################################
source('models/get_output_trialdata_varycourse_cocarriage5state.R')
clusterCall(cl, function() {source('models/get_output_trialdata_varycourse_cocarriage5state.R')})
...
for (resist in resistance.type) {
  for (abx in abx.type){
    ...
    if(all(names(sampled.para) == formalArgs(run_trialdata_varycourse_cocarriage5state))){
      ...
      trial.data.cocarriage = parLapply(cl, sampled.para.list, function(x) {
        run_trialdata_varycourse_cocarriage5state(n.bed = x['n.bed'], max.los  = x['max.los'], 
    ...
    save(dat, file = paste0("runs/", image_name, Sys.Date(), "_varycourse.Rdata"))
  }
}
