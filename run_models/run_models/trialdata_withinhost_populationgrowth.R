...
iterations = 20    # per set of parameters
sampled.para = data.frame(
  n.bed = rep(20, N),                         # n.bed; in this instance refers to number of repeats per set of parameters 
  n.day = rep(31, N),                
  ...
  K = runif(N, min = 18, max = 24), 
  total_prop = runif(N,min = 0.5, max = 0.9), # total proportion of holding capacity, the starting amount of enterobacteriaceae
  s_growth = runif(N, min = 0.001,max = 0.5),  # s_growth = amount transmitted on log scale
  fitness.r = runif(N, min = 0, max = 2), # fitness.r = growth constant for logistic growth
  r_trans = runif(N, min = 0.01, max = 0.2),  # r_trans = mean amunt of R transmitted
  abx.s = runif(N, min = 0.1, max = 0.5),     # abx.s = amount of s killed by broad spectrum abx s
...
# save data 
save(dat, file = paste0("runs/populationgrowth_", Sys.Date(), "_withinhost.Rdata"))
