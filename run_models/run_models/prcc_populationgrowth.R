...
parameters <- list(
  ...
  K = c("qunif", list(min=18, max=24)),            # gut holding capacity, on log scale, largest R number possible is exp(300) - typical colonic bacteria 10^14 number/mL content https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4991899/
  ...
  r_trans = c("qunif", list(min=0.01, max=0.2)),        # r_trans = mean amunt of R transmitted
  fitness.r = c("qunif", list(min=0, max = 3)),   # fitness.r = growth for R
  r_thres = c("qunif", list(min=0.01, max=0.2)),       # r_thres = threshold amount of bacteria before R can be transmitted
  s_growth = c("qunif", list(min=0.001,max=0.5)),   # s_growth = amount transmitted on log scale
  abx.s = c("qunif", list(min=0.1, max=0.8)),       # abx.s = amount of s killed by broad spectrum abx s
  abx.r = c("qunif", list(min=0.1, max=0.8)),       # abx.r = amount of r killed by broad spectrum abx r
