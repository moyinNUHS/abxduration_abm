...
clusterCall(cl, function() {source('models/model_cocarriage5state.R')})
...
modelRun.cocarriage <- function (data.df) { 
    return(mapply(run_model_cocarriage5state, 
...
parameters <- list(
    ...
    abx.r = c("qunif", list(min=0, max=0.5)),  #probability of clearing R to become r
    ...
    short_dur = c("qunif", list(min=3, max=7)),         #mean short duration of antibiotics (normal distribution) 
    long_dur = c("qunif", list(min=14, max=21))         #mean long duration of antibiotics (normal distribution) 
)
...
if(all(names(parameters) == formalArgs(run_model_cocarriage5state))){
    
    print('calculating 1st run')
    
    ## run 1 
    old = Sys.time() # get start time
    LHS.cocarriage = LHS(modelRun.cocarriage, factors, N = N, q, q.arg, nboot = 100, cl = cl) #N is the size of the hypercube
    ...
    N = N + 20
    old = Sys.time() # get start time
    LHS.cocarriage2 = LHS(modelRun.cocarriage, factors, N = N, q, q.arg, nboot = 100, cl = cl)
    print(Sys.time() - old) # print elapsed time difference in nice format
    image_name = paste0(model, "_", N, "_", abxr.effectiveness, "_", Sys.Date())
...
(mySbma <- sbma(LHS.cocarriage, LHS.cocarriage2))
