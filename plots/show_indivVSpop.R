...
w = ggplot(dw, aes(x = day, y = value, color = res)) + 
  ...
  scale_color_manual(values = c('#801E47', '#115570'), 
                     name = ' \n ',
...
arrow.dat = data.frame(y = c(0, r.eff , r.eff , 1, 0, r.ineff, 1, r.ineff ),
                       ...
                       abx = c(rep(ineff.lab, 4), 
                               rep(eff.lab, 4)))
lab.dat = data.frame(y = c(0.6, 0.15, 0.6, 0.12),
...
p = ggplot(dp, aes(x = Day, fill = variable, y = value)) +
  ...
  scale_fill_manual(values = rev(c('#E77EA5', '#801E47', '#f7e6e7', '#c5e2f0')), 
                    labels = c('Non-carriers', 'Selection', 'Transmission', 'Admitted as R carrier'),
...
ggsave('~/Documents/nBox/angelsfly/indiv_abxduration/manuscript/manuscript/graphs/fig3_indivVApop.jpg',
       width = 14,
       height = 10)
