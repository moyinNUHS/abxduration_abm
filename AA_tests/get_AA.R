...
## not in a function because filepath does not work within a function, and spartan is no longer maintained?

working.directory.main = getwd()

# CREATE FOLDER TO STORE OUTPUT 
model.folder = paste0('AA_tests/AA_runs/', model)
dir.create(model.folder, showWarnings = F)
...
if (model == 'simple3state'){
  ...
  parameters <- c(
    ...
    p.infect = c(runif(1, min=0.1, max=1)),     # "p.infect", probability of being prescribed antibiotics on admission
    p.infect.after = c(runif(1, min=0.001, max= 0.15)), # probability of being prescribed antibiotics after admission
  ...
  parameters <- c(
    ...
    fitness.r = c(runif(1,min=0, max=1)),  #probability of regrowth of s (sr—> sR)
  ...
  parameters <- c(
    ...
    p.r.day1 = c(runif(1,min=0.1, max=1)),     #probability of being prescribed broad spectrum antibiotic on day 1 of admission 
    p.r.after = c(runif(1, min=0.1, max=1)),    #  probability of being prescribed broad spectrum antibiotic after admission
    K = c(runif(1,min=exp(18), max=exp(24))),            # gut holding capacity, on log scale, largest R number possible is exp(300) - typical colonic bacteria 10^14 number/mL content https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4991899/
    ...
    r_trans = c(runif(1,min=0.01,max=0.5)),         # r_mean= R threshold level for tranmissibility
    fitness.r = c(runif(1,min=0.3, max=1.4)),   # fitness.r = growth constant for logistic growth
    r_thres = c(runif(1,min=0.01, max=0.2)),       # r_thres = threshold amount of bacteria before R can be transmitted
    s_growth = c(runif(1,min=0.03,max=0.3)),   # fitness.r = growth constant for logistic growth
...
if (model == 'simple3state'){
  aa_data = t(apply(params.matrix, 1, function(values) {
    run_absdiff_simple3state(n.bed = values[['n.bed']], max.los = values[['max.los']], 
                             ...
                             p.infect.after = values[['p.infect.after']], p.r.day1 = values[['p.r.day1']], p.r.after = values[['p.r.after']], short_dur = values[['short_dur']], long_dur = values[['long_dur']])
  }))
} else if (model == 'cocarriage5state') {
  aa_data = t(apply(params.matrix, 1, function(values) {
    run_absdiff_cocarriage5state(n.bed = values[['n.bed']], max.los = values[['max.los']], 
                                 ...
                                 bif = values[['bif']], pi_ssr = values[['pi_ssr']], repop.s = values[['repop.s']], fitness.r = values[['fitness.r']], mu = values[['mu']], 
                                 abx.s = values[['abx.s']], abx.r = values[['abx.r']], p.infect = values[['p.infect']], 
                                 p.infect.after = values[['p.infect.after']], p.r.day1 = values[['p.r.day1']], p.r.after = values[['p.r.after']], short_dur = values[['short_dur']], long_dur = values[['long_dur']])
  }))
  ...
  aa_data = t(apply(params.matrix, 1, function(values) {
    run_absdiff_populationgrowth(n.bed = values[['n.bed']], max.los = values[['max.los']], 
                                 p.infect = values[['p.infect']], p.infect.after = values[['p.infect.after']],
                                 p.r.day1 = values[['p.r.day1']], p.r.after = values[['p.r.after']], K = values[['K']], total_prop = values[['total_prop']], 
                                 prop_R = values[['prop_R']], pi_ssr = values[['pi_ssr']], r_trans = values[['r_trans']], fitness.r = values[['fitness.r']], 
...
setwd(model.folder)
model.abx.folder = paste0(getwd(),'/', abx.label)
dir.create(model.abx.folder, showWarnings = F)

for (i in iterationstotry){
  
  data = as.data.frame(out) # data to store
  setwd(model.abx.folder)    # go to the folder to store the data 
  ...
  for (k in 1 : numberofrepeatsineachiteration){
    ...
    setwd(paste0(model.abx.folder, '/', as.character(i))) # go to the folders for the `iterationstotry`
    dir.create(as.character(k), showWarnings = F) # create a folder for the repeat in the iteration 
    
    for (g in 1:i){
      setwd(paste0(model.abx.folder, '/', as.character(i), '/', as.character(k)))
      dir.create(as.character(g), showWarnings = F)
      setwd(paste0(model.abx.folder, '/', as.character(i), '/', as.character(k), '/', as.character(g)))
...
FILEPATH <- model.abx.folder #already in model.abx.folder stated above 
